...
def integrate(sentencedic,nodenum,intdic):
		...
		try:
			...
			for i in sorted(intdic):
				...
				sentencedic[i]['tag']=intdic[i]['tag']
			...
			if not intchi: intchi=intgov # by default same main token carries all
			...
			print ":::::::::::::::Error: function integrate was called with these parameters:"
			print sentencedic.dictrepr()
			print nodenum
			print intdic.dictrepr()
			print traceback.format_exc()+"\n\n\n"
...
def readinallmates(projectname,conlldirpath,filepattern="*.trees.conll14",eraseAllAnnos=True, steps=1000, importAnnotatorName=None, preserveSampleWithSameName=False):
	...
	files = []
	for root, dirs, filenames in os.walk(conlldirpath):
		for filename in fnmatch.filter(filenames, filepattern):
			files.append(os.path.join(root, filename).decode("utf-8"))
	print "found",len(files),"files to enter"
	...
	for filename in files:
		...
		for i in range(len(allsentences)/steps+1):
			...
			dbtextname = filename.split("/")[-1].replace("'","")
			...
			if dbtextname.endswith("-one-word-per-line.conll14_Parse"): # à adapter si besoin ou encore mieux généraliser de manière à ce que tout ce qui est devant le premier point suffit pour identifier un texte
...
def vietspacetomorph(conllfilename):
	"""
	puts tokens that have spaces into separate tokens connected by "morph" links to the previous token
	"""
	trees=conll.conllFile2trees(conllfilename)
	print len(trees),"sentences read from",conllfilename
	newtrees = []
		...
		for i in tree:
			...
			if " " in t:
				...
				for j,m in enumerate(t.split()):
					#print j,m
					iind = 'i'+str(j+1)
					insdic[iind]=copy.deepcopy(tree[i]) # copy all old features by default
					insdic[iind]['t']=m
					insdic[iind]['lemma']=m
					insdic[iind]['id']=iind
					insdic[iind]['gov']={'i'+str(j):'morph'}
				
				#print 'insdic',insdic.dictrepr(),"to put at position",i #str(insdic).encode("utf-8")	
				newtree = integrate(copy.deepcopy(newtree),i+addedpos,insdic)
				#print 'newtree',newtree.dictrepr()
				addedpos+=len(t.split())-1
				#print "addedpos",addedpos,t.split()
		print tree.sentencefeatures
		newtree.sentencefeatures=tree.sentencefeatures
		newtrees+=[newtree]
		#break
	conll.trees2conllFile(newtrees, conllfilename+'.morphed.conllu')
...
if __name__ == "__main__":
	print "bonjour"
	#vietspacetomorph("/home/kim/Documents/Mega/megasync/Viet/sud/sud.vi_vtb-ud-dev.conllu")
	#vietspacetomorph("/home/kim/Documents/Mega/megasync/Viet/sud/sud.vi_vtb-ud-test.conllu")
	#vietspacetomorph("/home/kim/Documents/Mega/megasync/Viet/sud/sud.vi_vtb-ud-train.conllu")
	...
	#readinallmates("Hiếu","../corpus/conll/",filepattern="Hieu009.conll10",steps=10000,importAnnotatorName="parser")
	readinallmates("linguistiqueCorpus-Echantillons-2018","../tools/corpussamples/",filepattern="*.conll_parse",steps=10000,importAnnotatorName="parser")
