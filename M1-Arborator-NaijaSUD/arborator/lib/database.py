...
class SQL:
	...
	def clean(self, cursor, table, columns, values):
		print("delete from "+table+"  where "+ " and ".join( [c+"=?" for c in  columns ] ) + ";",values)
	...
	def realname(self,user,crsr=None):
		...
		if not realna:
			...
			elif user in ["parser","mate","guest","gold", "katya"]:
				realna=user
			else: 
				realna = "anonymous"
	...
	def exportAnnotations(self, textid, textname, exportType):
		...
		if exportType in ["todosconll", "todosxml"]: 	# only todos are exported, __complete__ conll or xml files are created, non existing trees are filled up with parser trees
			...
			for _,uid,_,annotype,status,comment in self.getall(cursor, "todos",["textid"], [textid]): # for each assignment of the text to someone:
				...
				print("@@@@@", uid, annotype, status, comment, exportType, textid)
				a= self.getall(cursor, "users",["rowid"], [uid])
				if a :
					uid,user,realname = a[0]
					users+=[user]
	...
	def exportLastBestAnnotations(self, textid, textname):
		"""
		Conll export function (customized for Naija)

		Take the most recent tree and add the features and misc from a trusted user (here maigida, hardcoded in the function).
		If no misc (which contains timestamps info) can be found, copy those from parser.
		Also provides the rank of the tree in the text, to reorder them later on.

		input: Int, Str
		does: creates a file in projects/<project-name>/<textname>.most.recent.trees.with.feats.conllu
		output: List, Int
		"""
		print("exportingAnnotations")
		db,cursor=self.open()
		...
		if r:
			self.exportpath+u"é"
			basefilename+u"é"
			filename = self.exportpath+basefilename + u".most.recent.trees.with.feats.conllu"
			for sentenceid,nr,sentence,textid in r: # for each sentence of the text:
				...
				tree=self.conllSentenceExport(sentenceid, cursor, treeid, None)
				maigida_tree = self.gettree(sid=sentenceid, username="maigida")
				try:
					new_tree = addFeatsMiscs2Tree(tree, maigida_tree)
				except BaseException:
					print("problem with", maigida_tree)
					continue
				# print(maigida_tree)
				new_tree.sentencefeatures["nr"] = str(nr)

				if not hasMisc(new_tree):
					print("at least get timestamps from parser")
					base_tree = self.gettree(sid=sentenceid, username="parser")
					try:
						new_tree = addFeatsMiscs2Tree(tree, base_tree, miscOnly=True)
					except BaseException:
						print(tree, "\n\n", base_tree)
					new_tree.sentencefeatures["nr"] = str(nr)
		...
		print("all features and misc added")
		# users=["most recent modification (including trees from "+", ".join(users)+")"]
		conll.trees2conllFile(trees, filename)
		print("all done")
		return users,scounter
	
	# def exportLastTrusted(self, textid, textname, trusted, take_a_tree_always):
	# 	db,cursor=self.open()
	# 	scounter, fcounter, users=0,0,[]
	# 	doublegovs=False
	# 	renolet=re.compile("\W",re.U+re.I)
	...
	# 		self.exportpath+u"é"
	# 		basefilename+u"é"
	# 		filename = self.exportpath+basefilename + u".most.recent.by.trusted.conllu"
	# 		outf=codecs.open(filename.encode("utf-8"),"w","utf-8")

	# 		for sentenceid,nr,sentence,textid in r: # for each sentence of the text:

	# 			if take_a_tree_always:
	# 				treeDict=dict([ (user, (treeid,uid,realname,timestamp)) for treeid,uid,user,realname,timestamp in self.treesForSentence(sentenceid) if user in trusted])	
	# 				if not treeDict: # not annotation by a trusted annotator -> don't filter
	# 					treeDict=dict([ (user, (treeid,uid,realname,timestamp)) for treeid,uid,user,realname,timestamp in self.treesForSentence(sentenceid)])
	# 				# order by timestamp
	# 				timeDict=dict([ (timestamp,user) for (user, (treeid,uid,realname,timestamp)) in treeDict.iteritems() ])
	...
	def snippetSearch(self, query):
		...
		if featuresearch:
			...
			try:
				rc=cursor.execute("SELECT texts.textname, sentencesearch.rowid, sentences.nr, sentences.textid, snippet(sentencesearch) FROM sentencesearch, sentences, texts WHERE sentencesearch.sentence MATCH '%{query}%' AND sentencesearch.sentence=sentences.sentence AND sentencesearch.textid = texts.rowid;".format(query=query))
			except OperationalError:
				pass
		...
		counter=0
		allsid=list()
		# TODO : there's still a problem on the textname
		for tname, sid, snr, textid,r, in rc:

			if (textid, snr)in allsid:
				continue
			else:
				counter+=1
	...
	def correctSentenceLength(self,gooduserid=1, modifyDatabase=False):
		"""
			...
			for sid,snr,sentence,_ in self.getall(cursor, "sentences",["textid"], [textid]):
				...
				for tid in tidToToknr:
					if tidToToknr[tid]!=goodnrtoks:
						print "__sentence",snr,"good nb of tokens",goodnrtoks
						print "problem",tid,uid,"nr tokens:",tidToToknr[tid]
	...
	def removeStudentTrees(self):
        ...
        def removeTreesInTextByUser(self, textid, user):
		"""
		Useful to remove all tree by a certain user in a specific text

		@args:
		- numeric id of the text
		...
		q="""select trees.rowid as treeid, sentences.nr as sentencenr, users.user as username
			from trees, sentences, users
			where users.rowid = trees.userid and trees.sentenceid=sentences.rowid and sentences.textid={};""".format(textid)
		rc=cursor.execute(q)
		for treeid, sentencenr, username in rc:
			print treeid, sentencenr, username
			if username == user:
	...
	def removeSentence(self, textid, sid, transcription):
		db,cursor=self.open()
		trees = self.getall(cursor, "trees", ["sentenceid"], [sid])
		for t in trees:
			print("!")
			treeid,sid,uid,annoty,sta,comm,ti = t
			self.clean(cursor, "features", ["treeid"], [treeid])
			self.clean(cursor, "links", ["treeid"], [treeid])
			print("!!")
			self.clean(cursor, "trees", ["rowid"], [treeid])
			self.clean(cursor, "trees", ["rowid"], [treeid])
		self.clean(cursor, "sentencefeatures", ["sentenceid"], [sid])
		self.clean(cursor, "sentences", ["sentence", "textid"], [transcription, textid])
		db.commit()
		db.close()
		print("erase sentence {}".format(sid))
...
def addFeatsMiscs2Tree(baseTree, featTree, miscOnly=False):
	"""
	Copies the the features and misc fields from featTree onto baseTree.
	If called with miscOnly, only copies misc.

	input: Tree, Tree, Boolean
	output: Tree
	"""

	changed = False

	featTree = featTree.get("tree")
	if not featTree:
		raise BaseException("Weird tree format")

	try:
		assert len(baseTree) == len(featTree)
	except AssertionError:
		# print(baseTree)
		# print("\n\n")
		# print(featTree)
		raise BaseException("Les arbres n'ont pas la même longueur")

	for i in range(1, len(baseTree)+1):
		for k in featTree[i]:
			if not miscOnly:
				if k in ["xpos", "lemma", "tag", "tag2", "t", "upos", "egov", "gov", "id"]:
					continue
				else:
					changed = True
					baseTree[i].update({k:featTree[i][k]})
			else:
				if k == "misc":
					changed = True
					baseTree[i].update({k:featTree[i][k]})
...
def hasMisc(tree):
	"""
	Tests if the first node of a tree has something in its misc field.

	input: Tree
	output: Boolean
	"""

	misc = tree[1].get("misc", "_")
	if misc == "_":
		return False
	else:
		return True
...
if __name__ == "__main__":
	...
	trees = conll.conllFile2trees("../featspb.txt")
	# print(trees[1])
	addFeatsMiscs2Tree(trees[0], trees[1])

	qkkk
	#sql.removeStudentTrees()
	sql=SQL("NaijaSUD")
	#sql.removeTreesInTextByUser(60,"marine4maigida")
	#sql=SQL("Rhapsodie")
	#sql=SQL("lingCorpus")
	...
	#print sql.gettree(treeid=832)
	#print "_________"
	db,cursor=sql.open()
	#print sql.conllSentenceExport(181, cursor, 832, None)
	#print sql.conllSentenceExport(16, cursor, 806, None)
	#print sql.getAllSentences(textid=1, username="admin", userid=2, adminLevel=1, dbcrsr=(db,cursor))
	#print "iiiiiiiiiiiiiii"
	#print sql.getall(None, "sentences",["textid"],[1])
	#sql.correctSentenceLength(modifyDatabase=False)
	#sql.correctSentenceLength(modifyDatabase=True)
	#sql.cleanDatabase()
	...
	# def clean(self, cursor, table, columns, values):
	# 	cursor.execute("delete from "+table+"  where "+ " and ".join( [c+"=?" for c in  columns ] ) + ";",values)
	# test = sql.eraseTree(18432, 10611, "marine4maigida", 0)
	...
	# 		# fc,users,sc,doublegovs=self.exportAnnotations(tid, t, exportType )
	# 		print(t, tid)
	# 		if tid in [88, 89, 81, 90, 91, 86, 83, 8]:
	# 			continue
			if tid not in [71]:
				continue
			sql.exportLastTrusted(tid, t, ["marine4maigida", "marinecourtin", "maigida"], True)
	print("ok")
	# sql.removeSentence(64, 10616, '''I dey tell una sey [ if I enter church < father { do like dis |r do like dis |r do like dis } < { e don || e don } set for me >+ be dat ] //''')