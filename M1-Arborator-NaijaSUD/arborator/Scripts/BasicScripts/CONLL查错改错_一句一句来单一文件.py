...
f = open("P_ABJ_GWA_06_Ugo.lifestory_PRO.most.recent.trees.conll10","r")

#############################################################################
''' Find textname and textid'''
text_name = str(f)
text_name = text_name.replace("<_io.TextIOWrapper name='","") 
text_name = text_name.replace("' mode='r' encoding='cp65001'>","") 
text_name = text_name.replace(".most.recent.trees.conll10","") 
text_name = str(text_name.replace(".most.recent.trees.with.feats.conllu",""))
textnameid = re.findall("P_.+\d+_",text_name)
textnameid = str(textnameid[0])+"%"
